<%-- 
    Document   : addBook
    Created on : Jul 16, 2025, 12:59:26 PM
    Author     : HONG MINH
--%>
        ...
        <div class="form-container">
            ...
            <form action="MainController" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <c:if test="${published_yearError != null}">
                        <div class="text-danger">${published_yearError}</div>
                    </c:if>
                </div>
                <div class="mb-3">
                    <label for="total_copies" class="form-label">Total Copies</label>
                    <input type="number" class="form-control" id="total_copies" name="total_copies" value="${total_copies}">
                    <c:if test="${total_copiesError != null}">
                        <div class="text-danger">${total_copiesError}</div>
                    </c:if>
                </div>
                <div class="mb-3">
                    ...
                    <c:if test="${available_copiesError != null}">
                        <div class="text-danger">${available_copiesError}</div>
                    </c:if>
                    <c:if test="${availableError != null}">
                        <div class="text-danger">${availableError}</div>
                    </c:if>
                </div>
                <div class="mb-3">
